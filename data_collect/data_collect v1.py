...
def select_word_without_letters(word_list, letters, search_loc):
    for word in word_list:
        if all(letter not in word for letter in letters):
            print('Found {0} possiblities from {1}, Selected'.format(len(word),search_loc))
            return word.strip()
    return None
...
os.path.join(os.getcwd(), os.path.dirname(__file__), 'Training_Data')) #Getting current file directory
...
solve_time = []
file = open('Wordle\Algorithm_Output\wordle_data.csv', 'w', newline='')
writer = csv.writer(file)
writer.writerow(['Solution', 'Guesses[Indices]'])
for i in range(1):
    ...
    while True:
        ...
        if (current_guess == word_to_solve):
            ...
            for i, guess_ind in enumerate(previous_guesses):
                ...
                if i < len(previous_guesses) - 1:
                    writeRow += "/"
            writer.writerow([word_to_solve, writeRow])
        ...
        #print("Guess:{0} There are {1} Possible solutions".format(len(previous_guesses),len(solution_words)))
        iterations += 1
file.close()      
